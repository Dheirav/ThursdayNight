# Sai’s Thursday

A premium, colorful, romantic web app for couples to discover, plan, and remember movie nights together. Designed as a private, real-time, visually stunning streaming-like experience for Sai.

## Features
- Private shared room system (no login)
- Real-time sync via Supabase
- Role selection: “You” and “Sai”
- Add and save favorite movies/shows (TMDB API)
- Personalized recommendations
- Thursday voting system
- Weekly countdown
- Memory timeline
- Secret romantic section for Sai
- Premium, vibrant, glassmorphism UI

## Tech Stack
- Frontend: HTML, CSS, JavaScript (modular)
- Backend: Supabase (realtime)
- API: TMDB
- Hosting: Netlify or GitHub Pages

## Setup
1. Clone this repo
2. Create a [Supabase](https://supabase.com/) project
3. Get a [TMDB API key](https://www.themoviedb.org/documentation/api)
4. Add your Supabase and TMDB keys to `modules/config.js`
5. Run locally: Open `index.html` in your browser
6. Deploy: Use Netlify or GitHub Pages

## Folder Structure
- `index.html` – Main entry
- `styles.css` – Global styles
- `app.js` – App entry point
- `modules/` – Modular JS (features, UI, API, etc.)
- `assets/` – Images, icons, etc.

---

**Sai, this is our little world.**
